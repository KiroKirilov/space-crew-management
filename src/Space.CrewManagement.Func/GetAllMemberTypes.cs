...
public class GetAllMemberTypes(ILogger<GetAllMemberTypes> _logger, IMemberTypeService _memberTypeService)
{
    ...
    public async Task<IActionResult> Run([HttpTrigger(AuthorizationLevel.Function, "get", Route = "member-types")] HttpRequest req)
    {
        try
        {
            var licenses = await _memberTypeService.GetAll();
            return new OkObjectResult(licenses);
        }
        catch (Exception e)
        {
            _logger.LogError(e, "Error while getting member types.");
